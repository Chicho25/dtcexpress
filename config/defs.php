...
//	error_reporting(E_ERROR | E_WARNING | E_PARSE);
	...
	{
		$success= mysql_connect($GLOBALS["DB_Server2"], $GLOBALS["DB_Username2"], $GLOBALS["DB_Password2"]);
	...
	function MySQLQuery2($query)
	{

		$success= mysql_db_query($GLOBALS["DB_DBName2"], $query);
		if(!$success)
		{
			echo mysql_errno().": ".mysql_error()."<BR>";
			//MySQLQuery("ROLLBACK");
	...
	*/
	...
	function RecCountUltimaCotizacion($strTable, $strCriteria = "")
	{
		if(empty($strCriteria))
			$strQuery = "select count(*) as cnt from $strTable;";
		else
			$strQuery = "select count(*) as cnt from $strTable where stat = 5 and $strCriteria;";
	...
	*/
	...
	function InsertRec2($strTable, $arrValue)
	{
		$strQuery = "	insert into $strTable (";
